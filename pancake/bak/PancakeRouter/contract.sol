...
library PancakeLibrary {
    ...
    // 获取 tokenA & tokenB 所组建的 pair 的address (通过计算出来的)
    function pairFor(
        address factory,
        address tokenA,
        address tokenB
...
contract PancakeRouter is IPancakeRouter02 {
    ...
    address public immutable override WETH; // WBNB
    ...
    // 添加 token & BNB 的池子. 实际就是 token/WBNB
    function addLiquidityETH(
        address token, // erc20 token
        uint amountTokenDesired, //
        uint amountTokenMin, //
        uint amountETHMin, // BNB 的数量 和 msg.value 似乎一致
        address to, // 默认: tx.origin
        uint deadline // 时间要超过 block.timestamp
    )
        external
        payable
        virtual
