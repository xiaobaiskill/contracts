...
contract TransitSwapRouterV4 is Ownable, ReentrancyGuard, Pausable {
    ...
    event ChangeTransitSwap(
        address indexed previousTransit,
        address indexed newTransit
    );
    event ChangeTransitCross(
        address indexed previousTransit,
        address indexed newTransit
    );
    event ChangeTransitFees(
        address indexed previousTransitFees,
        address indexed newTransitFees
    );
    event ChangeSwapTypeMode(uint8[] types, bool[] newModes);
    event ChangeWrappedAllowed(address[] wrappeds, bool[] newAllowed);
    event TransitSwapped(
    ...
    constructor(
        address transitSwap_,
        address transitCross_,
        address transitFees_,
        address executor
    ) Ownable(executor) {
    ...
    function changeSwapTypeMode(uint8[] memory swapTypes)
        external
        onlyExecutor
    {
        bool[] memory newModes = new bool[](swapTypes.length);
        for (uint index; index < swapTypes.length; index++) {
            _swap_type_mode[swapTypes[index]] = !_swap_type_mode[
                swapTypes[index]
            ];
    ...
    function changeWrappedAllowed(address[] calldata wrappeds)
        external
        onlyExecutor
    {
        bool[] memory newAllowed = new bool[](wrappeds.length);
        for (uint index; index < wrappeds.length; index++) {
            _wrapped_allowed[wrappeds[index]] = !_wrapped_allowed[
                wrappeds[index]
            ];
    ...
    {
        if (preTradeModel) {
            fee = ITransitSwapFees(_transit_fees).getFeeRate(
                msg.sender,
                desc.amount,
                desc.swapType,
                desc.channel
        ...
        if (TransferHelper.isETH(desc.dstToken)) {
            if (preTradeModel) {
                ...
                if (desc.swapType == uint8(TransitStructs.SwapTypes.swap)) {
                    require(
                        _wrapped_allowed[desc.wrappedNative],
                        "TransitSwap: invalid wrapped address"
                    );
                    beforeBalance = IERC20(desc.wrappedNative).balanceOf(
                        address(this)
                    );
                } else {
            ...
            if (preTradeModel) {
                beforeBalance = IERC20(desc.dstToken).balanceOf(
                    desc.dstReceiver
                );
            } else {
        ...
        if (TransferHelper.isETH(desc.dstToken)) {
            if (preTradeModel) {
                ...
                require(
                    ...
                    "TransitSwap: insufficient return amount"
                );
            } else {
                if (desc.swapType == uint8(TransitStructs.SwapTypes.swap)) {
                    returnAmount = IERC20(desc.wrappedNative)
            ...
            if (preTradeModel) {
                returnAmount = IERC20(desc.dstToken)
                    .balanceOf(desc.dstReceiver)
                    .sub(beforeBalance);
                require(
                    returnAmount >= desc.minReturnAmount,
                    "TransitSwap: insufficient return amount"
                ...
                returnAmount = returnAmount.sub(fee);
                uint256 receiverBeforeBalance = IERC20(desc.dstToken).balanceOf(
                    desc.dstReceiver
                );
                TransferHelper.safeTransfer(
                    desc.dstToken,
    ...
    function cross(
        ...
        {
            //bytes4(keccak256(bytes('callbytes(TransitStructs.CallbytesDescription)')));
            (bool success, bytes memory result) = _transit_cross.call{
                value: swapAmount
            }(abi.encodeWithSelector(0xccbe4007, callbytesDesc));
            if (!success) {
                revert(RevertReasonParser.parse(result, "TransitSwap:"));
        ...
        if (!TransferHelper.isETH(desc.srcToken)) {
            require(
                IERC20(desc.srcToken).balanceOf(_transit_cross) >=
                    beforeBalance,
                "TransitSwap: invalid cross"
            );
        }
    ...
    function withdrawTokens(address[] memory tokens, address recipient)
        external
        onlyExecutor
    {
        for (uint index; index < tokens.length; index++) {
            ...
            if (TransferHelper.isETH(tokens[index])) {
                ...
                TransferHelper.safeTransferWithoutRequire(
                    tokens[index],
                    recipient,
                    amount
                );
